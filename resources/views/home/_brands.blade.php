...
<section id="brands" class="overflow-hidden bg-white py-24 sm:py-32">
    <div class="mx-auto max-w-7xl px-5 sm:px-8">
        <div class="reveal-section mx-auto mb-14 max-w-3xl text-center"><p class="section-kicker justify-center">{{ $sec->kicker ?? 'The names behind the music' }}</p><h2 class="section-title">@if($sec?->title){{ $sec->title }}@if($sec?->title_accent) <em>{{ $sec->title_accent }}</em>@endif@else Legendary makers.<br><em>One trusted destination.</em>@endif</h2></div>
    </div>
