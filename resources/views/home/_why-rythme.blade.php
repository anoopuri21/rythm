...
<section id="why-rythme" class="why-section bg-rythme-cream py-24 sm:py-32">
    <div class="mx-auto grid max-w-7xl gap-14 px-5 sm:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        ...
        <div>
            <div class="reveal-section"><p class="section-kicker">The Rythme standard</p><h2 class="section-title">More than a store.<br><em>Your partner in music.</em></h2><p class="mt-6 max-w-xl leading-7 text-rythme-warm-gray">Great instruments deserve great care. From expert selection to thoughtful delivery, every detail is designed around the musician.</p></div>
            <div class="mt-12 grid gap-x-8 gap-y-10 sm:grid-cols-2">
                @foreach($promises as $promise)
                    <article class="reveal-section group border-t border-black/10 pt-5">
                        ...
                        <h3 class="font-playfair text-xl">{{ $promise['title'] }}</h3><p class="mt-2 text-sm leading-6 text-rythme-warm-gray">{{ $promise['copy'] }}</p>
                    </article>
                @endforeach
            </div>
        </div>
